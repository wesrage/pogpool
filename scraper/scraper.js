...
export async function loadSchedule(year, season = Seasons.REGULAR) {
  const url = `https://statsapi.web.nhl.com/api/v1/schedule?season=${year}${year +
    1}`
  const { data: scheduleData } = await axios.get(url)
  const relevantGames = flattenSchedule(scheduleData).filter(
    game => game.gameType === season
  )
...
function parsePlayerStatsFromGameReport({
  liveData: {
    boxscore: { teams },
  },
}) {
